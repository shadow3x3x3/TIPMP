...
class SkyPath
  ...
  def attr_between(src, dst)
    @graph.edges_table[[src, dst]].attrs
  end
  ...
  def next_hop?(n, pass, next_path_attrs)
    unless @limit.nil?
      return false if out_of_limit?(next_path_attrs[1])
    end
  ...
  def query_check(src_id, dst_id)
    ...
    unless @graph.nodes.include?(src_id)
      raise ArgumentError, 'src id needs to exist Node'
    end
    unless @graph.nodes.include?(dst_id)
